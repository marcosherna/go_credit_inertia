...
import { ref } from 'vue';
import {
    PlusIcon, Table, ButtomToggleModal, FwInput,
    FwSelect, FwCheckbox, FwButton, FwTextArea, FwTooltip, FwModal,
    FwSearchBar, FwSkeleton, FwTable, FwBadge
...
lstChequeras.value = props.paginate.data
...
const onSearch = (text) => {
    lstChequeras.value = props.paginate.data.filter((item) => {
...
<template>
    <modulo-banco-layout title="Chequera">
        ...
        <Table>
            ...
            <template #body>
                <tr v-for="(chequera, index) in lstChequeras"
                    ...
                    <td class="px-6 py-4">
                        <fw-tooltip v-if="chequera.CHEQ_ESTADO == 1" @click="onSelected(chequera)"
                            class="bg-green-100 text-green-800 dark:text-green-400 border-green-400"
                            :label="chequeraEstado[chequera.CHEQ_ESTADO]" content="Precione para ver el detalle"
                            :target="chequeraEstado[chequera.CHEQ_ESTADO] + '-' + index" />

                        <fw-tooltip v-else-if="chequera.CHEQ_ESTADO == 0" @click="cambiarEstadoChequera(chequera.CHEQ_ID)"
                            class="bg-blue-100 text-blue-800 dark:text-blue-400 border-blue-400"
                            :label="chequeraEstado[chequera.CHEQ_ESTADO]" content="Precione para ver activar"
                            :target="chequeraEstado[chequera.CHEQ_ESTADO] + '-' + index" />

                        <fw-tooltip v-else class="bg-red-100 text-red-800 dark:text-red-400 border-red-400"
                            @click="onSelected(chequera)" :label="chequeraEstado[chequera.CHEQ_ESTADO]"
                    ...
                    <td class="px-6 py-4">
                        ...
                        <button v-if="chequera.CHEQ_ESTADO == 1" href="#" @click="seleccionarChequera(chequera)"
                            class="font-medium text-green-600 mr-2 dark:text-green-500 hover:underline">Emitir</button>

                        <button href="#" @click="selectedItem(chequera)"
                            class="font-medium text-green-600 dark:text-green-500 hover:underline">Editar</button>
                        <button v-if="chequera.CHEQ_ESTADO != 1" v-on:click="detelleCheque(chequera.CHEQ_ID)"
                            class="px-3 font-medium text-green-600 dark:text-green-500 hover:underline">Ver</button>
                    </td>
                </tr>
            </template>
        </Table>
    ...
    <fw-modal :show="isShowModal" maxWidth="sm" @close="isShowModal = false">
        <template #header>
                ...
                <form @submit.prevent="submit" class="p-4 md:p-5">
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2 sm:col-span-1">
                            <fw-input label="Desde" v-model:value="form.CHEQ_DESDE" type="number" :min="1"
                                placeholder="Desde" />
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            <fw-input label="Hasta" v-model:value="form.CHEQ_HASTA" :onUpdate:value="onchange"
                                :min="parseInt(form.CHEQ_DESDE)" placeholder="Hasta" type="number" />
                        ...
                        <div class="col-span-2">
                            <fw-select label="Cuenta de Bancos" v-model:selected="form.CUEB_NUMERO"
                                defaultItem="Seleccione una cuenta">
                                <option v-for="(cuenta, index) in cuentasBanco" :value="cuenta.CUEB_NUMERO"> {{
                                    cuenta.banco.BANC_NOMBRE }}</option>
                            </fw-select>
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            <fw-input label="Disponibles" v-model:value="form.CHEQ_CANTIDAD" :disabled="true" type="number"
                                placeholder="....." />
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            <fw-select label="Generacion" v-model:selected="form.CHEQ_GENERACION"
                        ...
                        <div class="col-span-2">
                            <fw-text-area label="Referencias" v-model:value="form.CHEQ_REFERENCIA"
    ...
    <fw-modal :show="isShowModalDetalle" maxWidth="3xl" @close="isShowModalDetalle = false">
        <template #header>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                chequera
            </h3>
                    ...
                    <tr v-for="(cheque, index ) in cheques" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex pb-2">
                                ...
                                <fw-badge>
                                    {{ controller.estadoCheque.find(x => x.value = cheque.CHEM_ESTADO) ?
    ...
    <fw-modal :show="emitirChequeModal" maxWidth="sm" @close="emitirChequeModal = false">
        <template #header>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                chequera
            </h3>
        ...
        <template #body>
            <form @submit.prevent="emitirCheque" class="p-4 md:p-5">
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2 ">
                        <fw-input label="No. Cheque" v-model:value="form_cheque.CHEM_NUMERO" type="number" :min="0"
                            :disabled="true" placeholder="Desde" />
                        <div v-show="form_cheque.errors.CHEM_NUMERO" class="text-red-500 text-sm">
                            {{ form_cheque.errors.CHEM_NUMERO }}
                        </div>
                    </div>
                    <div class="col-span-2 sm:col-span-1">
                        <fw-input label="Fecha" v-model:value="form_cheque.CHEM_FECHA" :onUpdate:value="onchange"
                            :min="parseInt(form.CHEM_FECHA)" placeholder="Hasta" type="date" />
                    ...
                    <div class="col-span-2 sm:col-span-1">
                        <fw-input label="Lugar" v-model:value="form_cheque.CHEM_LUGAR" placeholder="....." />
                    ...
                    <div class="col-span-2">
                        <fw-input label="Monto" v-model:value="form_cheque.CHEM_MONTO" :onUpdate:value="numerToLetter"
                            :min="0" :step="0.01" pattern="^\d+(?:\.\d{1,2})?$" type="number" placeholder="....." />
                    </div>
                    <div class="col-span-2">
                        <fw-input label="Monto Letras" v-model:value="form_cheque.CHEM_MONTOLETRA" :disabled="true"
                            placeholder="....." />
                    ...
                    <div class="col-span-2">
                        <fw-input label="Paguese a: " v-model:value="form_cheque.CHEM_NOMBRE" placeholder="....." />
                    ...
                    <div class="col-span-2">
                        <fw-text-area label="Comentarios: " v-model:value="form_cheque.CHEM_COMENTARIOS"
                            placeholder="Referencias...">
                        </fw-text-area>
